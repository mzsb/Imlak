...
class Field {
            ...
            case State.active:
                const onclick = () => {
                    ...
                    const hasInactiveNeighbour = [...this.getNeighbours()]
                        .some(f => (f === null || f === void 0 ? void 0 : f.state) === State.inactive);
    ...
    *getNeighbours() {
        if (this.y > 1)
            yield fields[this.x][this.y - 2];
        if (this.x > 1)
            yield fields[this.x - 2][this.y];
        if (this.y < height - 2)
            yield fields[this.x][this.y + 2];
        if (this.x < width - 2)
            yield fields[this.x + 2][this.y];
    ...
    for (let x = 0; x < height; x++) {
        ...
        for (let y = 0; y < width; y++) {
            ...
            if (mapParts.includes(x) || mapParts.includes(y)) {
                ...
                const isCenter = x === Math.floor(width / 2) &&
                    y === Math.floor(height / 2);
                rowFields.push(new Field(x, y, column, isCenter ? State.init : State.inactive));
            }
            else {
                rowFields.push(null);
            }
...
function size() {
    ...
    fieldSize = vWith / width > vHeight
        ? vWith / width
        : vHeight / height;
    const maxSize = Math.min(window.innerWidth / width, window.innerHeight / height) - 1;
    fieldSize = fieldSize >= maxSize ? maxSize : fieldSize;
    map.style.width = `${fieldSize * width}px`;
    map.style.height = `${fieldSize * height}px`;
    const first = fields.flat().filter((f) => f !== null)[0];
